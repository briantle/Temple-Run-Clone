...
public class LevelGenerator : MonoBehaviour {
	...
	void Update(){
		...
		if (Vector3.Distance(Camera.main.transform.position, currPosition) < distanceToInstantiate){
				Instantiate(platform, currPosition, platform.transform.rotation, gameObject.transform);
				currPosition += new Vector3(Random.Range(-2, 3), Random.Range(-1, 2), Random.Range(9, 13));
		}
	}
}
